...
export function Cart() {
  ...
  const handleInputChange = (field: keyof FormData, value: string) => {
    setFormData(prev => {
      ...
      // If switching from pickup to delivery and payment was cash, reset to payme
          ...
          <div className="text-center space-y-8">
            ...
            <div className="space-y-4">
              ...
              <p className="text-lg text-black/60">
                Добавьте часы в корзину, чтобы продолжить покупки
              </p>
            </div>
        ...
        {currentStep === 'cart' ? <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-16">
                  ...
                  <div className="flex-1 space-y-3 sm:space-y-4 min-w-0">
                    <div>
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <p className="text-[10px] sm:text-xs tracking-[0.2em] text-black/50 font-medium uppercase">
                          {item.collection}
                        </p>
                      </div>
                      <Link to={`/product/${item.id}`}>
                        <h3 className="text-base sm:text-lg font-semibold hover:text-[#C8102E] transition-colors">
                          {item.name}
                        </h3>
                      </Link>
                    </div>
            ...
            <div className="lg:col-span-1">
              <div className="border-2 border-black/10 p-6 sm:p-8 lg:sticky lg:top-24 space-y-6">
                ...
                <Link to="/catalog" className="block text-center text-sm text-black/60 hover:text-[#C8102E] transition-colors tracking-wider">
                  Продолжить покупки
                </Link>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
              ...
              <div className="lg:col-span-2 space-y-12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium tracking-wider uppercase mb-3">
                        ФИО <span className="text-[#C8102E]">*</span>
                      </label>
                      ...
                      {errors.fullName && <p className="text-red-500 text-xs mt-2">
                          {errors.fullName}
                        </p>}
                    </div>
                    <div>
                      ...
                      {errors.email && <p className="text-red-500 text-xs mt-2">
                          {errors.email}
                        </p>}
                    </div>
                    <div>
                      ...
                      {errors.phone && <p className="text-red-500 text-xs mt-2">
                          {errors.phone}
                        </p>}
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  ...
                  time: 'Сегодня',
                  cost: 0
                }].map(method => <button key={method.value} type="button" onClick={() => handleInputChange('deliveryMethod', method.value)} className={`p-6 border-2 text-left transition-all ${formData.deliveryMethod === method.value ? 'border-[#C8102E] bg-red-50' : 'border-black/20 hover:border-black/40'}`}>
                ...
                {/* Delivery Address */}
                {formData.deliveryMethod !== 'pickup' && <div className="space-y-6">
                    <div className="flex items-center space-x-4">
                      <div className="w-10 h-10 bg-black text-white rounded-full flex items-center justify-center font-semibold">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="md:col-span-2">
                        <label className="block text-sm font-medium tracking-wider uppercase mb-3">
                          Адрес <span className="text-[#C8102E]">*</span>
                        </label>
                        ...
                        {errors.address && <p className="text-red-500 text-xs mt-2">
                            {errors.address}
                          </p>}
                      </div>
                      <div>
                        <label className="block text-sm font-medium tracking-wider uppercase mb-3">
                          Город <span className="text-[#C8102E]">*</span>
                        </label>
                        ...
                        {errors.city && <p className="text-red-500 text-xs mt-2">
                            {errors.city}
                          </p>}
                      </div>
                      <div>
                        <label className="block text-sm font-medium tracking-wider uppercase mb-3">
                          Индекс <span className="text-[#C8102E]">*</span>
                        </label>
                        ...
                        {errors.postalCode && <p className="text-red-500 text-xs mt-2">
                            {errors.postalCode}
                          </p>}
                      </div>
                    </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    {availablePaymentMethods.map(method => <button key={method.value} type="button" onClick={() => handleInputChange('paymentMethod', method.value)} className={`w-full p-6 border-2 text-left transition-all flex items-center justify-between ${formData.paymentMethod === method.value ? 'border-[#C8102E] bg-red-50' : 'border-black/20 hover:border-black/40'}`}>
                        <div>
                  ...
                  {formData.deliveryMethod !== 'pickup' && <p className="text-xs text-black/50 italic">
